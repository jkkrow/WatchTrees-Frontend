import { VideoTreeClient, History, VideoNode } from 'store/slices/video-slice';

export const getAutoGeneratedThumbnail = (videoNode: VideoNode) => {
  let src = '';
  ...
  return `${process.env.REACT_APP_MEDIA_URL}/${videoNode.info.url.replace(
    /\.\w+$/,
    '.0000001.jpg'
  )}`;
};
